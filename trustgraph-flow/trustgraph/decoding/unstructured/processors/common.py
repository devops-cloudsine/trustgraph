"""
Common helpers for unstructured partitioning: normalization and auto-partition execution.
"""
...
# ---> partition_document > [partition_with_auto_and_normalize] > execute unstructured.auto.partition and normalize
def partition_with_auto_and_normalize(
	*,
	blob: bytes,
	...
	"""
	Logic preserved from the original monolithic implementation: call auto.partition
	and normalize the resulting element texts, with a minimal text-sample fallback.
	"""
	if partition is None:  # pragma: no cover - guarded by caller
		return []
